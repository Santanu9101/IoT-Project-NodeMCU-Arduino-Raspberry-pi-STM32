...
// Wi-Fi configuration
const char* ssid = "realme";              // Your Wi-Fi SSID
...
const char* ledTopic = "home/led";          // MQTT topic for LED status
const char* distanceTopic = "home/distance"; // MQTT topic for distance
...
void loop() {
    ...
    if (distance <= 50) { // Turn on LED if distance is 50 cm or less
        digitalWrite(ledPin, HIGH);
        mqttClient.publish(ledTopic, "ON");
    } else {
        digitalWrite(ledPin, LOW);
        mqttClient.publish(ledTopic, "OFF");
    }
